...
def main():
    ...
    with open("data/latest.json", "w", encoding="utf-8") as f:
        json.dump({"generated_at": datetime.now(KST).isoformat(), "count": len(results), "listings": results}, f, ensure_ascii=False, indent=2)

    with open("data/status.json", "w", encoding="utf-8") as f:
        json.dump({"last_run_kst": datetime.now(KST).isoformat(), "ok": True, "count": len(results)}, f, ensure_ascii=False, indent=2)
